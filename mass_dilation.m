...
d_end   = 2*r;            % [m] distance between center mass of orbs when they contact
g1      = 2*G*m/(d);  % initial relative acceleration for orbs
v       = 0.6;            % fraction of the speed of light of orbs
gamma_v = gamma(v);       % 1/sqrt(1-v^2/c^2)

% initialize other variables
dy = (d-d_end)/1e4;             % increment steps to numerical solution
ds = d:-dy:d_end;           % all numerical steps
...
subplot(2,1,2)
hold off
plot((d-ds)/2,100*percent_difference,'-b','LineWidth',2)
title({'% Difference'},'fontsize',16);
grid on
xlabel({'Distance Each Orb in Stationary Frame','Traveled Towards The Other [m]'},'FontSize',20);
